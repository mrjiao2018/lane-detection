...
class LaneNet(cnn_base_model.CNNBaseModel):
    """
    实现语义分割模型
    """
    def __init__(self, phase, net_flag='vgg'):
        ...
        if self._net_flag == 'vgg':
            ...
            self._encoder = dense_encoder.DenseEncoder(L=20, growth_rate=8,
                                                       with_bc=True,
                                                       phase=phase,
                                                       N=5)
    ...
    def compute_loss(self, input_tensor, binary_label, instance_label, name):
        ...
        with tf.variable_scope(name):
            ...
            # 像素嵌入
            pix_embedding = self.conv2d(input_data=decode_deconv, out_channel=4, kernel_size=1,
                                        use_bias=False, name='pix_embedding_conv')
            pix_embedding = self.relu(input_data=pix_embedding, name='pix_embedding_relu')
